"""
Stage 3: GRPO Training for Trajectory Generation — LLaMA-3.1-8B-Instruct
- 加载 Stage 2 SFT 的 LoRA checkpoint，merge 后作为起点
- 使用 acc/f1/edit_dist 指标与 ground_truth_schedule 对比作为 reward
"""
...
SFT_MODEL_PATH  = "/data/alice/cjtest/FinalTraj_arr/Trajectory_Generation_multi_agent/stage2_sft_llama_output_epoch10/final_model"
DATA_PATH       = "/data/alice/cjtest/FinalTraj_arr/Trajectory_Generation_multi_agent/stage1_training_data_3/metadata.jsonl"
OUTPUT_DIR      = "/data/alice/cjtest/FinalTraj_arr/Trajectory_Generation_multi_agent/stage3_grpo_llama_output"
MAX_SEQ_LENGTH  = 4096
MAX_PROMPT_LEN  = 2048
...
def compute_schedule_similarity(gen_schedule, gt_schedule) -> float:
    ...
    权重：accuracy 0.40 | act_type_sim 0.25 | macro_int_sim 0.25 | f1 0.10
    """
    ...
    if gen_int_sum > 0 and gt_int_sum > 0:
        gen_macro = np.sum(gen_act2int, axis=0) / gen_int_sum
...
def reward_format(completions, prompts=None, ground_truth_schedule=None, **kwargs) -> list:
    """格式奖励：含 [THOUGHT]+[JSON] → 1.0，含其一 → 0.5，都无 → 0.0"""
    scores = []
...
def reward_schedule_similarity(completions, prompts=None, ground_truth_schedule=None, **kwargs) -> list:
    """日程相似度奖励：与 ground_truth 对比（acc + F1 + int_sim）"""
...
def reward_constraints(completions, prompts=None, ground_truth_schedule=None, **kwargs) -> list:
    """基本约束奖励（纯结构性验证）：覆盖全天 / 连续不重叠 / 时长合理，各 1/3"""
    scores = []
    for completion in completions:
        resp = completion[0]["content"] if isinstance(completion, list) else completion
...
def build_prompt(user_profile: dict) -> list:
    """Return messages list; GRPOTrainer applies tokenizer's native chat template."""
...
def main():
    ...
    # 2. 加载 Base 模型 + 合并 SFT LoRA（全程 CPU，手动加载 safetensors）
