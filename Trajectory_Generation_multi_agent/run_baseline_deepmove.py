"""
DeepMove 基线推理脚本
从 metadata.jsonl 读取用户信息，使用预训练的 DeepMove 模型生成日程
输出格式与 run_inference.py 兼容，并自动调用评估函数

用法:
  python run_baseline_deepmove.py --data eval_data/california_50.jsonl
  python run_baseline_deepmove.py --data eval_data/arizona_50.jsonl
"""
...
# ─── 路径配置 ─────────────────────────────────────────────────────────────────
DEEPMOVE_DIR  = "/data/alice/cjtest/FinalTraj/Trajectory_Generation_tradition"
...
sys.path.insert(0, DEEPMOVE_DIR)
sys.path.insert(0, EVAL_DIR)

from deepmove_model import create_deepmove_model
...
def hour_to_time_string(hour: float) -> str:
    h = int(hour) % 24
...
def generate_schedule_from_sequence(activities, times):
    ...
    for i in range(len(activities) - 1):
        activity_name = IDX_TO_ACTIVITY.get(int(activities[i]), "home")
        start_hour = float(times[i])
        end_hour   = float(times[i + 1])
        if end_hour <= start_hour:
            end_hour = start_hour + 1
        schedule.append({
            "activity":   activity_name,
            "start_time": hour_to_time_string(start_hour),
            "end_time":   hour_to_time_string(end_hour),
        })
    last_name = IDX_TO_ACTIVITY.get(int(activities[-1]), "home")
    schedule.append({
        "activity":   last_name,
        "start_time": hour_to_time_string(float(times[-1])),
        "end_time":   "24:00",
    })
    return schedule
...
def main():
    ...
    # ── 加载特征词汇表 ────────────────────────────────────────────────────────
    vocab_path = os.path.join(DEEPMOVE_DIR, "feature_vocabs.json")
    with open(vocab_path) as f:
        feature_vocabs = json.load(f)
    print(f"Loaded feature vocabs ({len(feature_vocabs)} features)")

    # ── 创建并加载模型 ────────────────────────────────────────────────────────
    model = create_deepmove_model(ACTIVITY_TYPES, feature_vocabs, device=device)
    ckpt_path = os.path.join(DEEPMOVE_DIR, "checkpoints", "best_model.pt")
    ...
    persons_list = []
    for s in samples:
        p = dict(s["user_profile"])   # copy
        p["user_id"] = s["user_id"]   # ensure user_id is at top-level
    ...
    # ── 推理 ──────────────────────────────────────────────────────────────────
    dataset = InferenceDataset(persons_list, feature_vocabs)
    loader  = DataLoader(dataset, batch_size=args.batch, shuffle=False,
                         collate_fn=collate_inference_fn)

    all_results = []
    with torch.no_grad():
        for batch in tqdm(loader, desc="DeepMove inference"):
            user_ids       = batch["user_ids"]
            start_acts     = batch["start_activities"].to(device)
            ...
            acts_seq, times_seq = model.generate_sequence(
                start_acts, start_times, user_features,
    ...
    gen_path = os.path.join(RESULTS_DIR, f"generated_deepmove_{n}samples_{ts}.json")
    ...
    try:
        ...
        eval_dir = os.path.join(RESULTS_DIR, f"eval_deepmove_{data_tag}_{ts}")
