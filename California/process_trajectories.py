...
# 读取数据
print("正在读取数据...")
trips_df = pd.read_csv('nhts17-caltrans-tsdc-download/data/survey_trip.csv', low_memory=False)
household_df = pd.read_csv('nhts17-caltrans-tsdc-download/data/survey_household.csv', low_memory=False)
value_lookup_df = pd.read_excel('nhts17-caltrans-tsdc-download/data/Value_Lookup.xlsx')

print(f"读取了 {len(trips_df)} 条出行记录")
print(f"读取了 {len(household_df)} 条家庭记录（仅用于获取 travday 和日期字段）")

# 筛选工作日和节假日
print("\n预筛选: 只保留工作日且非节假日的记录...")

# 只保留工作日出行的家庭 (travday 在 1-5)
...
print(f"   工作日筛选前: {before_filter} 个家庭")
print(f"   工作日筛选后: {after_filter} 个家庭 (移除了 {before_filter - after_filter} 个家庭)")

# 筛选节假日：移除出行日期在节假日的家庭（使用holiday字段）
if 'holiday' in household_df.columns:
    before_filter = len(household_df)
    household_df = household_df[household_df['holiday'] != 1]
    after_filter = len(household_df)
    print(f"   节假日筛选前: {before_filter} 个家庭")
    print(f"   节假日筛选后: {after_filter} 个家庭 (移除了 {before_filter - after_filter} 个家庭)")
    print(f"   使用 holiday 字段进行筛选")
else:
    # 如果没有 holiday 字段，使用 holidays 包自动获取美国联邦节假日（2016-2017年）
    print("   警告: 未找到 holiday 字段，使用 holidays 包自动获取节假日")
...
# 只保留筛选后的家庭的出行记录
valid_sampno = household_df['sampno'].unique()
trips_df = trips_df[trips_df['sampno'].isin(valid_sampno)]

print(f"\n筛选后保留的出行记录数: {len(trips_df)}")
print(f"筛选后保留的家庭数: {len(valid_sampno)}")
...
# 读取 person 数据获取工作状态
person_df = pd.read_csv('nhts17-caltrans-tsdc-download/data/survey_person.csv', low_memory=False)
...
# 保存需要移除的个人ID列表
invalid_sampno_file = 'data_person/invalid_sampno_from_trajectory.json'
with open(invalid_sampno_file, 'w', encoding='utf-8') as f:
    json.dump(invalid_sampno_list, f, indent=2)
print(f"已保存需要移除的个人ID列表到: {invalid_sampno_file}")
...
# 保存结果
output_file = 'data_person/california_trajectories_processed.json'
