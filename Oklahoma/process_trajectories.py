...
# 读取数据
print("正在读取数据...")
trips_df = pd.read_csv(r'E:\FrankYcj\FinalTraj\Oklahoma\2017-tsdc-nhts-incog-add-on-download\data\survey_trip.csv', low_memory=False)
household_df = pd.read_csv(r'E:\FrankYcj\FinalTraj\Oklahoma\2017-tsdc-nhts-incog-add-on-download\data\survey_household.csv', low_memory=False)
value_lookup_df = pd.read_excel(r'E:\FrankYcj\FinalTraj\Oklahoma\2017-tsdc-nhts-incog-add-on-download\data\Value_Lookup.xlsx')
...
# 读取 person 数据获取工作状态
person_df = pd.read_csv(r'E:\FrankYcj\FinalTraj\Oklahoma\2017-tsdc-nhts-incog-add-on-download\data\survey_person.csv', low_memory=False)
...
# 5. 筛选活动个数 >= 7 的轨迹
print("\n步骤 5: 筛选活动个数 >= 7 的轨迹...")
invalid_persons_too_many_activities = []

for person_id in trips_df['person_id'].unique():
    person_trips = trips_df[trips_df['person_id'] == person_id]
    # 统计该个人的出行记录数（即活动个数）
    activity_count = len(person_trips)
    if activity_count >= 7:
        invalid_persons_too_many_activities.append(person_id)

if len(invalid_persons_too_many_activities) > 0:
    invalid_sampno_from_activities = trips_df[trips_df['person_id'].isin(invalid_persons_too_many_activities)]['sampno'].unique()
    before_count = len(invalid_sampno_set)
    invalid_sampno_set.update(invalid_sampno_from_activities)
    after_count = len(invalid_sampno_set)
    print(f"   找到 {len(invalid_persons_too_many_activities)} 个人的活动数 >= 7")
    print(f"   涉及 {len(invalid_sampno_from_activities)} 个家庭")
    print(f"   新增移除: {after_count - before_count} 个家庭 (累计: {after_count})")
else:
    print(f"   没有发现活动数 >= 7 的个人")
...
# 保存需要移除的 sampno 列表
invalid_sampno_file = r'E:\FrankYcj\FinalTraj\Oklahoma\processed_data\invalid_sampno_from_trajectory.json'
...
# 保存结果
output_file = r'E:\FrankYcj\FinalTraj\Oklahoma\processed_data\oklahoma_trajectories_processed.json'
